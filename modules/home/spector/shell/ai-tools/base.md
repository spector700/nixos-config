...
If a binary/program is not installed locally, `nix-shell` to run the tool you
need for troubleshooting or one-off tasks. The program does not need to be on
`PATH`.

## Rapidly-Evolving Topics

For any question involving technology that changes faster than training data
updates — AI/LLM models, framework versions, tool comparisons, package
ecosystems — do NOT answer from training data alone.

Before answering, either:
1. Use `web_search_exa` for conceptual/semantic queries, or `WebSearch` with current year/month for freshness-critical lookups, OR
2. Delegate to the researcher agent

For questions about a specific GitHub repository (architecture, how a module works, codebase structure), use `deepwiki` before fetching raw files.

This applies to: "which model should I use", "what's the best X", "is X still
maintained", "how does A compare to B" — anywhere a newer release could
invalidate a training-data answer.

Asserting a version-specific fact (e.g. "X is the best model for your use case")
without a web citation is a failure mode.
