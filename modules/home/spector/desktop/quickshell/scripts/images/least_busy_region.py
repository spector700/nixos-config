...
def find_least_busy_region(
    ...
    if screen_width is not None and screen_height is not None:
        ...
        if verbose:
            print(
                f"Scaling image from {orig_w}x{orig_h} to {new_w}x{new_h} (scale: {scale:.3f}, mode: {screen_mode})"
            )
...
def find_largest_region(
    ...
    min_size = 10
    max_size = (
        min(h, int(w / aspect_ratio))
        if aspect_ratio >= 1.0
        else min(int(h * aspect_ratio), w)
    )
    best = None
    best_size = min_size
...
def draw_region(
    ...
    x, y = coords
    cv2.rectangle(
        img, (x, y), (x + region_width - 1, y + region_height - 1), (0, 0, 255), 3
    )
    cv2.imwrite(output_path, img)
    print(f"Saved output image with rectangle at {output_path}")


def draw_largest_region(
    image_path,
    center,
    size,
    output_path="output.png",
...
def get_dominant_color(
    image_path, x, y, w, h, screen_width=None, screen_height=None, screen_mode="fill"
):
    img = cv2.imread(image_path)
    if img is None:
        raise FileNotFoundError(f"Image not found: {image_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Find least busy region in an image and output a JSON. Made for determining a suitable position for a wallpaper widget."
    )
    ...
    parser.add_argument(
        "-v", "--visual-output", action="store_true", help="Output image with rectangle"
    )
    parser.add_argument(
        "--screen-width",
        type=int,
        default=1920,
        help="Screen width for wallpaper scaling",
    )
    parser.add_argument(
        "--screen-height",
        type=int,
        default=1080,
        help="Screen height for wallpaper scaling",
    )
    parser.add_argument(
        "--stride",
        type=int,
        default=10,
        help="Step size for sliding window (higher is faster, less precise)",
    )
    parser.add_argument(
        "--screen-mode",
        choices=["fill", "fit"],
