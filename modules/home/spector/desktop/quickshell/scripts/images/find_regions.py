...
def find_regions(
    ...
    if resize_factor != 1.0:
        image = cv2.resize(
            image,
            (int(orig_w * resize_factor), int(orig_h * resize_factor)),
    ...
    for x, y, w, h in rects:
        ...
        if w > min_width and h > min_height:
            if (max_width is None or w < max_width) and (
...
def main():
    ...
    parser.add_argument(
        "--min-width", type=int, default=200, help="Minimum width of detected region"
    )
    parser.add_argument(
        "--min-height", type=int, default=100, help="Minimum height of detected region"
    )
    parser.add_argument(
        "--max-width", type=int, help="Maximum width of detected region"
    )
    parser.add_argument(
        "--max-height", type=int, help="Maximum height of detected region"
    )
    parser.add_argument(
        "--single",
        action="store_true",
        help="Only output the most likely (largest) region",
    )
    parser.add_argument(
        "--quality",
        action="store_true",
        help="Use quality mode for selective search (slower, less sensitive)",
    )
    parser.add_argument(
        "--k", type=int, default=3000, help="Segmentation parameter k (default: 150)"
    )
    parser.add_argument(
        "--min-size",
        type=int,
        default=50,
        help="Segmentation parameter min_size (default: 20)",
    )
    parser.add_argument(
        "--sigma",
        type=float,
        default=0.6,
        help="Segmentation parameter sigma (default: 0.8)",
    )
    parser.add_argument(
        "--resize-factor",
        type=float,
        default=0.1,
        help="Resize factor for input image before processing (default: 1.0, e.g. 0.5 for half size)",
    )
    parser.add_argument(
        "--hyprctl",
        action="store_true",
    ...
    if args.hyprctl:
        regions = [
            {"at": [r["x"], r["y"]], "size": [r["width"], r["height"]]} for r in regions
        ]
