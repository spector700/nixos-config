...
# copy modules/*.ctl to the config

echo -e "\n\n// sourced from modules based on config.ctl //\n" >>$conf_file
echo "$write_mod" | sed 's/","/\n/g ; s/ /\n/g' | awk '!x[$0]++' | while read mod_list; do
	...
	if [ -f $modules_dir/$mod_cpy.ctl ]; then
		output=$(eval "cat <<EOF
        $(cat "$modules_dir/$mod_cpy.ctl")
        EOF
        ")
...
cat $modules_dir/footer.ctl >>$conf_file
